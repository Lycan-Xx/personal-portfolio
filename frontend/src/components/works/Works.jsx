...
const ProjectCard = ({ project, index, inView, isLeft }) => {
  ...
  return (
    ...
    >
      ...
      <div className={`absolute top-1/2 w-8 h-0.5 bg-gradient-to-r from-cyan-400/60 to-cyan-400/20 ${
        isLeft ? 'right-0' : 'left-0'
      }`} />
      
      {/* Status dot at branch connection */}
      <div className={`absolute top-1/2 -translate-y-1/2 w-3 h-3 ${statusConfig.dotColor} rounded-full border-2 border-gray-900 ${
        isLeft ? '-right-1.5' : '-left-1.5'
      }`} />

      {/* Project Card */}
      <div className="glass-card group hover:shadow-xl hover:shadow-cyan-400/10 transition-all duration-300 overflow-hidden max-w-md">
        ...
        <div className="p-6">
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {(project.tags || []).slice(0, 3).map((tag, i) => (
              <span
                key={i}
                className="px-2 py-1 text-xs font-mono font-medium text-cyan-400 bg-cyan-400/10 border border-cyan-400/20 rounded-md"
              >
                {tag}
              </span>
            ))}
            {(project.tags?.length || 0) > 3 && (
              <span className="px-2 py-1 text-xs font-mono font-medium text-gray-400 bg-gray-800/50 border border-gray-600/30 rounded-md">
          ...
          <div className="flex gap-3">
            <a
              ...
              className="flex-1 flex items-center justify-center gap-2 px-4 py-2 bg-cyan-400/10 hover:bg-cyan-400/20 border border-cyan-400/30 hover:border-cyan-400/50 rounded-lg text-cyan-400 font-medium transition-all duration-200 hover:scale-105"
            >
            ...
            {project.repo && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex-1 flex items-center justify-center gap-2 px-4 py-2 bg-gray-800/50 hover:bg-gray-700/50 border border-gray-600/30 hover:border-gray-500/50 rounded-lg text-gray-300 hover:text-white font-medium transition-all duration-200 hover:scale-105"
              >
      ...
      <div className="w-full max-w-6xl mx-auto relative">
        ...
        <div className="relative p-6 md:p-10 z-10">
          ...
          <div className="relative max-w-4xl mx-auto">
            ...
            {/* Main Branch Line */}
            <div className="absolute left-1/2 top-0 bottom-0 w-0.5 bg-gradient-to-b from-cyan-400/80 via-cyan-400/60 to-cyan-400/40 transform -translate-x-1/2">
              {/* Animated glow effect */}
              <div className="absolute inset-0 w-1 bg-cyan-400/20 blur-sm transform -translate-x-1/4"></div>
            </div>
                ...
                return (
                  <div key={project.id || index} className="relative">
                    ...
                    <motion.div
                      ...
                      className="absolute left-1/2 top-1/2 w-4 h-4 bg-cyan-400 rounded-full border-4 border-gray-900 transform -translate-x-1/2 -translate-y-1/2 z-20"
                    >
                      <div className="absolute inset-0 bg-cyan-400 rounded-full animate-ping opacity-20"></div>
                    </motion.div>

                    {/* Project Card positioned on alternating sides */}
